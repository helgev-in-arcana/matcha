...
fn main() {
    ...
    // Only web builds embed a font. A native build enumerates real system
    // fonts, so fetching one would be several megabytes and a network round
...
/// Obtaining the font that `src/embedded_font.rs` embeds.
///
/// The font is **not** in this repository: it is ~9.6 MB of binary that would
/// live in git history forever, and this repository is the framework, not a
/// site. A page built on matcha subsets the font to its own text in its own
/// repository; the demos here just want every character to exist.
///
/// So it is downloaded once and cached next to the source (gitignored). To use
/// a font you already have — a subset, or a different family entirely — point
/// `MATCHA_EMBEDDED_FONT` at it and nothing is downloaded.
mod font {
    use super::*;

    /// Pinned to a release tag rather than a branch, so the bytes a build gets
    ...
    pub fn ensure_available() {
        ...
        if let Some(path) = std::env::var_os("MATCHA_EMBEDDED_FONT") {
            ...
            // Copied rather than embedded from its original location so that
            // `include_bytes!` has one fixed path to name.
            write(&dest, &bytes);
            println!("cargo:warning=embedding the font from {}", path.display());
            return;
        }

        // A cached file is trusted on size alone: re-hashing 9.6 MB on every
        // build to detect a corruption that only a manual edit could cause is
        // not worth the second it costs.
    ...
    /// The pinned revision's length, used as the cheap "already downloaded"
    /// check. Kept next to [`SHA256`] so the two are updated together.
    ...
    /// Downloads via `curl`, which ships with Windows 10+, macOS and every
    /// mainstream Linux.
    ///
    /// Shelling out rather than taking an HTTP client as a build-dependency:
    /// build dependencies compile for the host on *every* build of this crate,
    /// including the native ones that never want a font at all, and a TLS stack
    /// is not a small thing to make them pay for.
    ...
    /// SHA-256, so that pinning a revision actually pins the bytes.
    ///
    /// Written out rather than pulled in for the same reason `download` shells
    /// out: a hash of one file at build time does not justify a dependency
    /// every native build would also compile. FIPS 180-4.
