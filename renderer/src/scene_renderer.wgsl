...
@fragment fn color(in: VertexOut) -> @location(0) vec4<f32> {
    var local = 1.0;
    if (params.masked & 2u) != 0u { local = clamp(textureSample(local_coverage, image_sampler, in.uv).r, 0.0, 1.0); }
    return textureSample(source_image, image_sampler, in.uv) * params.opacity * mask_at(in.position) * local;
}
@fragment fn mask(in: VertexOut) -> @location(0) vec4<f32> {
    let c = clamp(textureSample(source_image, image_sampler, in.uv).r, 0.0, 1.0) * mask_at(in.position);
